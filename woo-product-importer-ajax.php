...
if (isset($post_data['uploaded_file_path'])) {
    ...
    if (intval($post_data['header_row']) == 1)
    ...
    foreach ($import_data as $row_id => $row) {
        ...
        foreach ($row as $key => $col) {
            ...
            switch ($map_to) {
                ...
                case 'sku_parent':
                    if (!wc_get_product_id_by_sku($col))
                        continue;
            ...
            switch ($map_to) {
                ...
                case '_file_path':
                case '_file_paths':
                    if (!is_array($new_post_meta['_file_paths']))
                        $new_post_meta['_file_paths'] = array();
                ...
                case 'custom_field':
                    ...
                    $new_post_custom_fields[sanitize_title($field_name)] = array(
                        ...
                        "is_visible" => $visible,
                        "is_variation" => in_array($post_data['type_product'], ['product_variation', 'variable_product']) ? 1 : 0,
        ...
        if (!$new_post['sku_parent'] && in_array($post_data['type_product'], ['product_variation'])) {
            ...
            if (strlen($new_post['post_title']) > 0 || $existing_product !== null) {
                ...
                if ($existing_product !== null) {
                    ...
                    if ('product_variation' == $post_data['type_product']) {
                        //it is variation
                        $new_post['post_type'] = 'product_variation';

                        if (isset($new_post['sku_parent']) && $product_id = wc_get_product_id_by_sku($new_post['sku_parent'])) {
                            $isVariable = true;
...
//        $menuOrder[$product_id] ++;
    ...
    $new_post['post_title'] = $product->get_title();
